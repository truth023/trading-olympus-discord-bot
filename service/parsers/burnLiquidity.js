...
async function parseBurnTransaction(sig) {
  try {
    ...
    if (ix_index == -1) {
      for (let i = 0; i < inner_ixs.length; i++) {
        ...
        // TODO: need to check the inner ix is burn liquidity ix
        if (idx > -1) {
    ...
    inner_ixs
      ...
      .map((inn_ix) => {
        ...
        if (owner === raydiumAuthority.toBase58())
          bIsSending = ACTION_TYPE.WITHDRAW;
        else if (owner === feePayer || inn_ix.parsed?.type == "burn")
          bIsSending = ACTION_TYPE.BURN;
        result.push({
          mode: bIsSending,
          user: feePayer,
          amount: inn_ix.parsed?.info?.amount,
          mintOrAta: inn_ix.parsed?.info?.mint || inn_ix.parsed?.info?.source,
          signature: sig,
        });
      });
...
module.exports = { parseBurnTransaction };
